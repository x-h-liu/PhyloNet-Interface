...
import PostProcessingModule.MCMCNetworkDisp
...
class PMCMCGTPage(QDialog):
    def __init__(self, fname, credibleSet, parent=None):
        ...
        """
        super(PMCMCGTPage, self).__init__(parent)
    ...
    def initUI(self):
        ...
        if root.find("command").text != "MCMC_GT":
            raise RuntimeError
        ...
        for param in root.find("parameters"):
            if param.tag == "cl":
                ...
                title = QLabel("Starting networks:")
            elif param.tag == "tm":
                title = QLabel("Gene tree / species tree taxa association:")
            elif param.tag == "pseudo":
                title = QLabel("Use pseudo likelihood instead of full likelihood to reduce runtime:")
            else:
                continue
    ...
    def showNetworks(self):
        ...
        # Generate inputs(list for overallmap and list of tuples for 95% credible set) for MCMCNetworkDisp
...
if __name__ == '__main__':
    ...
    ex = PMCMCGTPage("/Users/liu/Desktop/testdata/xml/MCMC_GT.xml", "/Users/liu/Desktop/testdata/xml/CredibleSet.xml")
    ex.show()
    sys.exit(app.exec_())
