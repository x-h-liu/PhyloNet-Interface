...
class PNetworkMLPage(QDialog):
    def __init__(self, fname, parent=None):
        super(PNetworkMLPage, self).__init__(parent)
    ...
    def initUI(self):
        ...
        if root.find("command").text != "InferNetwork_ML":
            raise RuntimeError
        ...
        # Values
        commandText = QLabel("InferNetwork_ML")
        ...
        for param in root.find("parameters"):
            if param.tag == "numReticulations":
                ...
                title = QLabel("Maximum consecutive number of failures for hill climbing:")
            elif param.tag == "b":
                ...
                title = QLabel("After the search the returned species networks will be optimized for their branch "
                               "lengths and inheritance probabilities:")
            elif param.tag == "o":
                ...
                title = QLabel("Use the branch lengths of the gene trees for the inference:")
            else:
                continue
        ...
        for network in root.findall("network"):
            ...
            # disp = MLNetworkDisp.MLNetworkDisp(network[0].text, network[1].text, self)
            # disp.show()
...
if __name__ == '__main__':
    ...
    ex = PNetworkMLPage("/Users/liu/Desktop/testdata/xml/InferNetwork_ML2.xml")
    ex.show()
    sys.exit(app.exec_())
